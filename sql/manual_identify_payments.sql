...
select zmluva, f.*, z.* from faktury f join zakaznici z on z.id = id_zak where zmluva in (1616);
...
select * from zakaznici where email like '%kubelet%' or meno like '%kubelet%' or telefon like '%0918%513%287%';

select zmluva, p.* from platby p left join zakaznici z on id_customer = z.id 
where iban = 'SK8711000000002933663155' or account like '%2933663155' 
	or sender like '%wolf%' 
	or popis like '%wolf%' 
order by datum_platby;

select zmluva, p.* from yndev.platby_new p left join zakaznici z on id_customer = z.id 
where (iban = 'SK8902000000003996136154' or account like '%3996136154' 
	or sender like '%vaskova%' 
	or popis like '%vaskova%') #and suma = 10
order by datum_platby;
