...
###### store customer balance to tables
	...
	drop table if exists yidev.customer_balance_detail;

	create table yidev.customer_balance_detail as
	SELECT 
	...
	FROM yidev.uhrady_new as u
	left join yidev.faktury_source as f on f.id = u.id_faktura
	left join yidev.platby_source as p on p.id = u.id_platba
	left join sluzby as s on s.id = f.id_sluzby
	join zakaznici as z on u.id_zak = z.id
	;

	drop table if exists yidev.customer_balance;

	create table yidev.customer_balance as
    ...
    from yidev.customer_balance_detail as d
    join (
            ...
            from yidev.customer_balance_detail 
            group by id_zak) as m 
		on d.id_zak = m.id_zak
		and d.step_f = m.max_step_f
		and d.step_p = m.max_step_p
	join (select id_zak, sum(suma) as total_invoiced from yidev.faktury_source group by id_zak) as f on f.id_zak = d.id_zak
	join (select id_customer, sum(suma) as total_paid from yidev.platby_source group by id_customer) as p on p.id_customer = d.id_zak
	join zakaznici as z on z.id = d.id_zak
	order by meno
	;

	# select * from yidev.customer_balance_detail where id_zak = 175

	select * from yidev.customer_balance_detail order by meno limit 100;
    select * from yidev.customer_balance order by meno;
