drop procedure if exists yidev.match_payments_to_invoices;

delimiter $$

create procedure yidev.match_payments_to_invoices()
begin


############ create steps table

	drop table if exists yidev.customer_steps;

	create table yidev.customer_steps
	(
...
############ create extended invoice_payments
	...
	drop table if exists yidev.uhrady_new;


	create table yidev.uhrady_new like yellownet.uhrady;
	alter table yidev.uhrady_new add suma_p decimal(10,2);
	alter table yidev.uhrady_new add suma_f decimal(10,2);
	alter table yidev.uhrady_new add datum date;
	alter table yidev.uhrady_new add id_zak int;
	alter table yidev.uhrady_new add balance decimal(10,2);
	alter table yidev.uhrady_new add step_f int;
	alter table yidev.uhrady_new add step_p int;

############ init first step one time

	truncate table yidev.uhrady_new;
	truncate table yidev.customer_steps;

	insert into yidev.customer_steps
	select distinct id_zak, 1, 1, 1, 1, 0 
	from yidev.faktury_source
    #where id_zak = 102
    ;

	set @rowcount = row_count();
    
    update yidev.customer_steps as cs
    join (select id_zak, max(step) as step_max from yidev.faktury_source group by id_zak) as f on f.id_zak = cs.id_zak
    set cs.steps_f = f.step_max
    ;
    
    update yidev.customer_steps as cs
    join (select id_customer, max(step) as step_max from yidev.platby_source group by id_customer) as p on p.id_customer = cs.id_zak
    set cs.steps_p = p.step_max
    ;
    
	#select * from yidev.customer_steps;
    #select * from yidev.platby_source
...
############ loop
	...
	while @rowcount > 0 do
		...
		select 
			...
			cs.step_f, 
			cs.step_p
		from yidev.customer_steps as cs
        left join yidev.faktury_source as f on f.id_zak = cs.id_zak and f.step = cs.step_f
		left join yidev.platby_source as p on p.id_customer = cs.id_zak and p.step = cs.step_p
		where coalesce(f.id, p.id) is not null
		order by id_zak, step_f, step_p
		#limit 1
		;
        
        #select * from yidev.customer_steps as c left join yidev.platby_source p on p.id_customer = c.id_zak and p.step = c.step_p;
		...
		update yidev.customer_steps as cs
		join yidev.uhrady_new as u on u.id_zak = cs.id_zak
