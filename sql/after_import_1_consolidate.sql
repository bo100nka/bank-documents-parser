...
###### iban_payer to vs
	...
	insert into yndev.import_lookup_ibanpayer2vs values (-1,'SK6711000000002616088134_Krčová Monika');
	...
	insert into yndev.import_lookup_ibanpayer2vs values (-1,'SK8302000000001783078158_MGR. FRANTISEK JANKOVIC - SINET');
	insert into yndev.import_lookup_ibanpayer2vs values (-1,'SK8109000000000251536520_Michala Michalkova');
	...
	insert into yndev.import_lookup_ibanpayer2vs values (-1,'SK6802000000004456937457_Peter Jarabek');
...
###### consolidation
	...
	select payer_iban, payer_name, amount
    ,concat('insert into yndev.import_lookup_ibanpayer2vs values (-1,''', payer_iban, '_', trim(payer_name), ''');') as insert_statement
    from yndev.import_consolidated 
    where vs = 0 and is_credit = 1 
    group by payer_iban
    order by 2;
